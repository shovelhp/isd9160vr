...
{
	/* The maximum clock rate of the Delta-Sigma Converter is 6.144MHz.
	Best performance is gained with clocks rates between 1.024MHz and 4.096MHz*/
	//Don't use 384 over sample, performance is not good!
	
	/* suggestion sample rate for HCLK = 49.152MHz*/
	/////////////////////////////////////////////////////////////////////
	//  ADC CLKDIV |    SD_CLK     |     Sample Rate (Hz) for OSR	   //
	//			   |               |   	 64		    128		    192    //
 	/////////////////////////////////////////////////////////////////////
	//		16			3,072,000		48,000		24,000	    16,000 //
	//		24			2,048,000		32,000		16,000	    10,667 //
	//		32			1,536,000		24,000		12,000	    8,000  //		
	/////////////////////////////////////////////////////////////////////
	
	UINT32 u32SD_Clk; 
	UINT16 au16OSR[3] = {64, 128, 192};
	//UINT16 au16OSR[3] = {192, 128, 64};
	UINT8 u8Div, u8OSRIdx = 2;
	...
	do{		
		...
		u32SD_Clk = SystemCoreClock/u8Div;
		if ((u32SD_Clk>1024000) && (u32SD_Clk<4096000))
			break;
	}while(++u8OSRIdx<3);
	
	// Only check best performance, suggest to change sample rate if condition meet
	if ((u32SD_Clk<=1024000) || (u32SD_Clk>=4096000))
		while(1);
...
void MIC_Open(void)
{
   ...
   ADC_SetALCTargetLevel(ADC_ALC_TARLEV_USE);
//	ADC_SetALCTargetLevel(-600); //ALCLVL , -6.0db;  //DARREN
	ADC_SET_ALCATTACKTIME(ADC, 2);
	ADC_SET_ALCDECAYTIME(ADC, 3);
//ADC_ENABLE_ZEROCROSSING();
	
	ADC_ENABLE_NOISEGATE(ADC, ADC_ALCCTL_NGPEAK_P2P);
	ADC_SET_NOISEGATE_TH(ADC, ADC_ALCCTL_NGTH_USE);
#endif
/*
#if USEADCALC == 0
	ADC_DISABLE_ALC(ADC);
...
//	ADC_ENABLE_PGA(ADC, ADC_PGACTL_REFSEL_VMID, ADC_PGACTL_BOSST_GAIN_0DB);
  	ADC_ENABLE_PGA(ADC, ADC_PGACTL_REFSEL_VMID, ADC_PGACTL_BOSST_GAIN_USE);
	// ADC_SetPGAGaindB(ADC_PGA_GAIN); 
	ADC_SetPGAGaindB(ADC_PGA_GAIN_USE); 
	
	/* MIC circuit configuration */
	ADC_ENABLE_VMID(ADC, ADC_VMID_HIRES_DISCONNECT, ADC_VMID_LORES_CONNECT);
	ADC_EnableMICBias(ADC_MICBSEL_USE);
