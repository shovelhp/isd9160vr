...
#define MD4APP_OUT_FRAME_NUM	  		10													// It can be : 2, 4, 6, ....
...
// 1: Enable  to duplicate decoded PCM to other target
// 0: Disable to duplicate decoded PCM to other target to have a small RAM and ROM
#define MD4APP_DUPLICATE_ENABLE	1
...
typedef struct
{
	...
	// Duplicate data into buffer or callback function
	S_BUF_CTRL_CALLBACK sDuplicateTarget;
#endif
...
{
	BufCtrl_SetCallback(&psMD4App->sDuplicateTarget, (PFN_DATA_REQUEST_CALLBACK)BufCtrl_WriteWithCount, (void*)psOutBufCtrl);
}
...
{
	BufCtrl_SetCallback(&psMD4App->sDuplicateTarget, pfnFunc, pParamOfFunc);
}
